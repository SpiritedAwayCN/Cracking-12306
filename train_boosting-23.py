...
from models.BoostingModel2_ResNet import ResNetv2
from models.BoostingModel3_SeNet import ResNetSE
...
if __name__=='__main__':
    ...
    if len(sys.argv)!=2:
        print('usage: python train_boosting-23.py <boosting_model_index>')
        exit()
    if not sys.argv[1] in ['2','3']:
        print('this script is for boosting model 2&3 only')
        exit()
    if sys.argv[1]=='2':
        model = ResNetv2()
        model_name = 'Boosting2-ResNet'
        train_mdata_path = './metadata/train_label2.txt'
        test_mdata_path = './metadata/test_label2.txt'
    else:
        model = ResNetSE()
        model_name = 'Boosting3-SeNet'
        train_mdata_path = './metadata/train_label3.txt'
        test_mdata_path = './metadata/test_label3.txt'

    model.build(input_shape=(None,) + c.input_shape)

    train_iter = get_train_dataset(train_mdata_path).__iter__()

    warmup(model, train_iter)
    model.save_weights(model_name + '-warmup.h5')
    test(model,test_mdata_path)
    ...
    for epoch in range(0, c.total_epoches):
        ...
        if epoch % 10 == 0 or epoch >=40:
            model.save_weights(model_name+('-{:0>2}.h5'.format(epoch + 1)))
        test(model,test_mdata_path)