...
class ResNetv2M2(tf.keras.models.Model):
    def __init__(self, **kwargs):
        super(ResNetv2M2, self).__init__(**kwargs)

        self.conv0 = Conv2D(16, (5, 5), strides=(1, 1), name='conv0', padding='same', use_bias=False, kernel_initializer='he_normal')
        self.block_collector = []
        block_num = 6, 6, 6
        filters_num = 16, 32, 64
        for i in range(1, 4):
...
class ResNetv2M4(tf.keras.models.Model):
    def __init__(self, **kwargs):
        super(ResNetv2M4, self).__init__(**kwargs)

        self.conv0 = Conv2D(32, (6, 6), strides=(1, 1), name='conv0', padding='same', use_bias=False, kernel_initializer='he_normal')
        self.block_collector = []
        block_num = 6, 6, 6
        filters_num = 32, 32, 64
        for i in range(1, 4):
            if i == 1:
                self.block_collector.append(BasicBlock(filters_num[i-1], name='conv1_0'))
            else:
