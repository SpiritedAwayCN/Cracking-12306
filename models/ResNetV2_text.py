...
class BasicBlock(tf.keras.layers.Layer):
    def __init__(self, filters, lamb=0, strides=(1, 1), **kwargs):
...
class ResNetv2_text(tf.keras.models.Model):
    def __init__(self, num_class, lamb=0, **kwargs):
        super(ResNetv2_text, self).__init__(**kwargs)
        ...
        for i in range(1, 4):
            if i == 1:
                self.block_collector.append(BasicBlock(filters_num[i-1], lamb=lamb, name='conv1_0'))
            else:
                self.block_collector.append(BasicBlock(filters_num[i-1], lamb=lamb, strides=(2, 2), name='conv{}_0'.format(i)))

            for j in range(1, block_num[i-1]):
                self.block_collector.append(BasicBlock(filters_num[i-1], lamb=lamb, name='conv{}_{}'.format(i, j)))

        self.bn = BatchNormalization(name='bn', momentum=0.9, epsilon=1e-5)
        self.activation = Activation('relu')
        self.global_average_pooling = GlobalAvgPool2D()
        self.fc = Dense(num_class, name='fully_connected', activation='softmax', use_bias=False, kernel_initializer='he_normal',
            kernel_regularizer=l2(lamb))
...
def main():
    model = ResNetv2_text(80)
    model.build((None, ) + (57, 57, 1))
